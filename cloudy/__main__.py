...
def test():
    ...
    assert cloudy.config_from_file(c_path) == dict(foo='bar')
...
def watch(to_watch: str, config: Optional[str] = None):
    cfg = cloudy.config_from_file(Path(config) if config else _config_path())
    ...
    handler = _compose(
        ...
        partial(
            cloudy.bitly_shorten,
            cfg['bitly_token'],
            cfg['web_root']
        ),
        cloudy.copy_to_clipboard,
        cloudy.show_notification,
    )

    notifier = cloudy.watch_dir(Path(to_watch), handler=handler)
